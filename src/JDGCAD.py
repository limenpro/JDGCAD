...
class Model:
    ...
    def set_up(self, model_name, topk, size):
        ...
        model_config = {
            ...
            'phi-3': {
                'end_token_ids': [13, 29871, 259, 32000],  # '\n' tokens, ' ', '<|endoftext|>'
            }
        # If you want to use JDGCAD on models from other families,
        ...
        self.detector = joblib.load(f'{path_prefix}-top{topk}-size{size}-model-mlp.pkl')

        with open(f'{path_prefix}-top{topk}-size{size}-para-lr.pkl', 'rb') as f:
            data = pickle.load(f)
            # 不用LR模型系数，维度对不上
    ...
    def get_utilization_distribution(self, topk_attentions, context_start_idx, context_end_idx, token_level_probability_distribution):
        ...
        unique_context_attention_ratio = unique_context_attention / unique_context_attention_sum  # [topk_head, len(unique_context_ids)] [H, U]

        # 1.3 detect utilized-tokens
        scaler = self.detector._scaler
        data = unique_context_attention_ratio.permute(1, 0).cpu().numpy() # [U, H]
        data = np.nan_to_num(data, nan=0.0)
        data_scaled = scaler.transform(data)
        # 用预测概率替换mask
        probs = torch.tensor(self.detector.predict_proba(data_scaled), device=self.device, dtype=torch.float32) # [len(unique_context_ids),]

        # 1.4 Top-Rank Constraint
        ranks = self.get_context_token_rank(token_level_probability_distribution, unique_context_ids)
        probs[ranks > self.top_rank] = 0

        # step2: calculate utilization distribution
        utilization_distribution = torch.zeros(self.vocab_size, device=self.device)
        if probs.sum() > 0:
            detector_output = self.detector.predict(data) # [len(unique_context_ids),]
            informative_head = torch.from_numpy(detector_output).to(probs.device).float() * probs[:,1] # 提取正类
    ...
    def generate(self,question: str, context: str, gen_max_length: int = 10, use_jdgcad=True):
        '''
        Parameters:
            question: question
            context: context
            gen_max_length: Max new token length
            use_jdgcad: Whether to use JDGCAD
        Returns:
            generated token ids
        '''
            ...
            for cur_len in range(gen_max_length):
                ...
                if use_jdgcad:
                    ...
                    if gen_max_length == 1 and next_token in self.end_token_ids:
                        ...
                        # 凸组合+重新归一化
                        p_model = token_level_probability_distribution          # Tensor [V] 或 [1,V]
                        p_util  = utilization_distribution                      # Tensor 同 shape
                        lam = self.alpha * self.normalized_entropy(p_model)     # 这里可能是 float
                        
                        # 关键：转成 tensor，并对齐 device/dtype
                        lam = torch.as_tensor(lam, device=p_model.device, dtype=p_model.dtype)
                        
                        gamma = 1.0 - torch.exp(-lam)
                        final_distribution = (1 - gamma) * p_model + gamma * p_util
                        final_distribution = final_distribution / (final_distribution.sum(dim=-1, keepdim=True) + 1e-12)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--size', type=int, default=2000, help='LR_MLP-train-size')
    ...
    for i, data in enumerate(tqdm(datas)):
        ...
        # JDGCAD answers
        jdgcad_tokens = model.generate(
            question=question,
            context=context,
            gen_max_length=tgt_len,
            use_jdgcad=True
        )
        if jdgcad_tokens is None: continue
        pred_ans = model.tokenizer.decode(jdgcad_tokens, skip_special_tokens=True)
    ...
    save_path = f'../results/{args.model}-mlp/{args.data}-{args.size}.jsonl'
    # 如果目录不存在，则创建
    ...
    ############# greedy results #############
    if greedy:
        greedy_f1, _, _ = utils.evaluate_f1(results, gold_ans_key='gold_ans', pred_ans_key='greedy_ans')
        greedy_em, _, _ = utils.evaluate_em(results, gold_ans_key='gold_ans', pred_ans_key='greedy_ans')
    ...
    ############# jdgcad results #############
    jdgcad_f1, _, _ = utils.evaluate_f1(results, gold_ans_key='gold_ans', pred_ans_key='pred_ans')
    jdgcad_em, _, _ = utils.evaluate_em(results, gold_ans_key='gold_ans', pred_ans_key='pred_ans')
    logger.info('----- JDGCAD performance -----')
    logger.info(f'EM: {jdgcad_em * 100}%')
    logger.info(f'F1: {jdgcad_f1 * 100}%')
    logger.info('*' * 30)

    print('----- JDGCAD performance -----')
    print(f'EM: {jdgcad_em * 100}%')
    print(f'F1: {jdgcad_f1 * 100}%')

    with open(f'../results/{args.model}-mlp/{args.data}-top{args.topk}-size{args.size}.txt', 'a') as file:
        if greedy:
            file.write('----- Greedy performance -----\n')
            file.write(f'EM: {greedy_em * 100}%\n')
            file.write(f'F1: {greedy_f1 * 100}%\n\n')

        file.write('----- JDGCAD performance -----\n')
        file.write(f'EM: {jdgcad_em * 100}%\n')
        file.write(f'F1: {jdgcad_f1 * 100}%')