...
def sample_data(sample_num, train_data_dir='../datasets/train/'):
    ...
    # you can sample training data from each subset 'HotpotQA', 'NaturalQuestionsShort', 'NewsQA', 'SearchQA', 'SQuAD', 'TriviaQA-web', "NQ-swap"
    # num = int(sample_num / 6)
    # data_list = ['HotpotQA', 'NewsQA', 'SearchQA', 'SQuAD', 'TriviaQA-web', "NQ-swap"]
...
def train_detector(model_name, topk=10, train_data_size=100):
    ...
    Train a detector using:
      - Logistic Regression for feature selection (to get top-k important features)
      - Single-layer MLP (ReLU) as the final classifier on selected features
    input: (n_samples,topk)
    detector.predict: (n_samples,)
    detector.predict_prob: (n_samples,2)
    
    :param model_name: the name of the LLM
    :param topk: how many top features used for training
    :param train_data_size: total training data size (half positive, half negative)
    :return: None (saves model and parameters to disk)
    '''
    ...
    def train_and_select_best_mlp(X, y, param_grid, n_splits, topk_features=None):
        '''
        Train MLP with k-fold CV and select best model based on average validation performance.
        Applies StandardScaler per fold to avoid data leakage.
        '''
        all_models = []
        ...
        # First pass: train one model per fold
        for fold, (train_idx, test_idx) in enumerate(cv.split(X, y), start=1):
            ...
            # Attach scaler to model for later use (or save separately)
            best_model._scaler = scaler  # custom attribute
        ...
        # Second pass: evaluate each model on ALL folds' test sets
        for model in all_models:
            ...
            for train_idx, test_idx in cv.split(X, y):
                ...
                if topk_features is not None:
                    X_test = X_test[:, topk_features]
                X_test_scaled = model._scaler.transform(X_test)  # use its own scaler
                y_pred = model.predict(X_test_scaled)
                val_accuracies.append(accuracy_score(y_test, y_pred))
            model_average_accuracies.append(np.mean(val_accuracies))

        best_idx = np.argmax(model_average_accuracies)
        best_model = all_models[best_idx]
        print(f"Best Model Selected from Fold {best_idx + 1} with Average ACC: {model_average_accuracies[best_idx]:.4f}")
        return best_model

    # === Load data ===
    data = pd.read_parquet(f'../datasets/{model_name}/feature_vector.parquet')
    X = data.iloc[:, :-1].to_numpy()  # features
    y = data.iloc[:, -1].to_numpy()   # labels (0 or 1)

    # === Step 1: Use Logistic Regression to select top-k features ===
    print("\n===== Selecting Top-K Features using Logistic Regression =====")
    X_balanced_lr, y_balanced_lr = balance_data(X, y, random_state=42, sample_size=train_data_size // 2)

    lr = LogisticRegression(max_iter=1000, penalty='l2', C=1.0, random_state=42)
    lr.fit(X_balanced_lr, y_balanced_lr)

    coefficients = lr.coef_[0]
    topk_indices = np.argsort(np.abs(coefficients))[-topk:]

    # === Step 2: Train MLP on selected top-k features ===
    print("\n===== Training MLP with Top-K Features =====")
    X_balanced_mlp, y_balanced_mlp = balance_data(X, y, random_state=43, sample_size=train_data_size // 2)
    ...
    best_topk_model = train_and_select_best_mlp(
        X_balanced_mlp, y_balanced_mlp,
        param_grid=param_grid_mlp,
        n_splits=5,
        topk_features=topk_indices
    )
    ...
    topk_model_path = f'../detector/{model_name_clean}-top{topk}-size{train_data_size}-model-mlp.pkl'
    joblib.dump(best_topk_model, topk_model_path)
    print(f"Saved Top-{topk} MLP model to {topk_model_path}")

    # Since MLP doesn't have interpretable coefficients, we save the LR-based weights for reference
    lr_weights_on_topk = coefficients[topk_indices]
    abs_weights = np.abs(lr_weights_on_topk)
    normalized_weights = (abs_weights / abs_weights.sum()).tolist() if abs_weights.sum() > 0 else [1.0 / topk] * topk

    data_to_save = {
        'topk_indices': topk_indices.tolist(),
        'lr_based_weights': normalized_weights  # named clearly to avoid confusion
    }
    para_filename = f'../detector/{model_name_clean}-top{topk}-size{train_data_size}-para-lr.pkl'
    with open(para_filename, 'wb') as f:
        pickle.dump(data_to_save, f)

    print(f"Top-{topk} feature indices and LR-based normalized weights saved in {para_filename}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_data_size', type=int, default=2000, help='total training data size for training the detector (half positive, half negative)')
    ...
    print('\n\nstep4: train context utilization detector')
    train_detector(model_name, topk=10, train_data_size=args.train_data_size)